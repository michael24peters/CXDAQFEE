...
!panel: TFCTop.pnl
    state: NOT_READY
    !color: FwStateAttention1
        when ( any_in SODINTYPE_FWSETSTATES in_state ERROR ) move_to ERROR
        when ( all_in SODINTYPE_FWSETSTATES in_state READY ) move_to READY
        when ( all_in SODINTYPE_FWSETSTATES in_state RUNNING )  move_to RUNNING
        when (  ( all_in SODINTYPE_FWSETSTATES in_state ACTIVE )  )  move_to READY
        when (  ( all_in SODINTYPE_FWSETSTATES in_state ACTIVE )  )  move_to ACTIVE
        when ( any_in SODINTYPE_FWSETSTATES in_state PAUSED ) move_to PAUSED
        action: CONFIGURE(int STEP_NR = -1, string RUN_TYPE = "PHYSICS", string RUN_INFO_DP = "")	!visible: 1
            do CONFIGURE(STEP_NR=STEP_NR,RUN_TYPE=RUN_TYPE,RUN_INFO_DP=RUN_INFO_DP) all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state READY ) then
                move_to NOT_READY
            endif
            move_to READY
        action: RESET	!visible: 1
            do RESET all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state NOT_READY ) then
                move_to READY
            endif
            move_to NOT_READY
    state: READY
    !color: FwStateOKNotPhysics
        when ( any_in SODINTYPE_FWSETSTATES in_state ERROR ) move_to ERROR
        when ( any_in SODINTYPE_FWSETSTATES in_state NOT_READY ) move_to NOT_READY
        when ( all_in SODINTYPE_FWSETSTATES in_state RUNNING )  move_to RUNNING
        when (  ( all_in SODINTYPE_FWSETSTATES in_state ACTIVE )  )  move_to ACTIVE
        when (  ( all_in SODINTYPE_FWSETSTATES in_state PAUSED )  )  move_to PAUSED
        when ( any_in SODINTYPE_FWSETSTATES in_state PAUSED ) move_to PAUSED
        action: START_RUN(int N_TRIGGERS = 0, int STEP_NR = 0, float STEP_PERIOD = 0.0, string AUTO_STEP_FILE = "")	!visible: 0
            do START_RUN(N_TRIGGERS=N_TRIGGERS, STEP_NR=STEP_NR, STEP_PERIOD=STEP_PERIOD, AUTO_STEP_FILE=AUTO_STEP_FILE) all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state {ACTIVE,RUNNING} ) then
                move_to READY
            endif
            move_to ACTIVE
        action: START(int N_TRIGGERS = 0, int STEP_NR = 0, float STEP_PERIOD = 0.0, string AUTO_STEP_FILE = "")	!visible: 1
            do START(N_TRIGGERS=N_TRIGGERS, STEP_NR=STEP_NR, STEP_PERIOD=STEP_PERIOD, AUTO_STEP_FILE=AUTO_STEP_FILE) all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state {ACTIVE,RUNNING} ) then
                move_to READY
            endif
            move_to ACTIVE
    ...
    !color: FwStateOKNotPhysics
        when ( any_in SODINTYPE_FWSETSTATES in_state ERROR )  move_to ERROR
        when ( any_in SODINTYPE_FWSETSTATES in_state NOT_READY )  move_to NOT_READY
        when ( any_in SODINTYPE_FWSETSTATES in_state READY )  move_to READY
        when ( any_in SODINTYPE_FWSETSTATES in_state PAUSED ) move_to PAUSED
        when ( all_in SODINTYPE_FWSETSTATES in_state RUNNING )  move_to RUNNING
        action: STOP_RUN	!visible: 0
            do STOP_RUN all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state READY ) then
                move_to RUNNING
            endif
            move_to READY
        action: STOP	!visible: 1
            do STOP all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state READY ) then
                move_to RUNNING
            endif
            move_to READY
        action: START_TRIGGER(int N_TRIGGERS = 0, int STEP_NR = 0, float STEP_PERIOD = 0.0, int RUN_NUMBER = 0)	!visible: 1
            do START_TRIGGER(N_TRIGGERS=N_TRIGGERS, STEP_NR=STEP_NR, STEP_PERIOD=STEP_PERIOD, RUN_NUMBER=RUN_NUMBER) all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state RUNNING ) then
                move_to ACTIVE
            endif
            move_to RUNNING
        action: START_RUN	!visible: 0
            do START_RUN all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state RUNNING ) then
                move_to READY
            endif
            move_to RUNNING
        action: START(int N_TRIGGERS = 0, int STEP_NR = 0, float STEP_PERIOD = 0.0, string AUTO_STEP_FILE = "")	!visible: 0
            ...
            move_to ACTIVE
        action: START_SCAN	!visible: 0
            do START_SCAN all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state READY ) then
                move_to RUNNING
            endif
            move_to READY
        action: STOP_SCAN	!visible: 0
            do STOP_SCAN all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state READY ) then
                move_to RUNNING
            endif
            move_to READY
    ...
    !color: FwStateOKNotPhysics
        when ( any_in SODINTYPE_FWSETSTATES in_state ERROR ) move_to ERROR
        when ( any_in SODINTYPE_FWSETSTATES in_state NOT_READY ) move_to NOT_READY
        when ( any_in SODINTYPE_FWSETSTATES in_state ACTIVE )  move_to ACTIVE
        when ( any_in SODINTYPE_FWSETSTATES in_state READY ) move_to READY
        when ( all_in SODINTYPE_FWSETSTATES in_state RUNNING )  move_to RUNNING
        action: CONTINUE(int N_TRIGGERS = 0, float STEP_PERIOD = 0.0)	!visible: 1
            do CONTINUE(N_TRIGGERS=N_TRIGGERS, STEP_PERIOD=STEP_PERIOD) all_in SODINTYPE_FWSETACTIONS
            if ( any_in SODINTYPE_FWSETSTATES not_in_state RUNNING ) then
                move_to PAUSED
            endif
            move_to RUNNING
        action: STOP_RUN	!visible: 0
        ...
        action: STOP	!visible: 1
            ...
            move_to READY
        action: STOP_TRIGGER	!visible: 1
            do STOP_TRIGGER all_in SODINTYPE_FWSETACTIONS
            if ( any_in SODINTYPE_FWSETSTATES not_in_state ACTIVE ) then
                move_to RUNNING
            endif
            move_to ACTIVE
    state: RUNNING
    !color: FwStateOKPhysics
        ...
        action: STOP_TRIGGER	!visible: 1
            ...
            move_to ACTIVE
        action: PAUSE	!visible: 1
            do PAUSE all_in SODINTYPE_FWSETACTIONS
    ...
    state: ERROR
    !color: FwStateAttention3
        when ( all_in SODINTYPE_FWSETSTATES not_in_state ERROR ) move_to NOT_READY
        action: RECOVER	!visible: 1
            do RECOVER all_in SODINTYPE_FWSETACTIONS
            if ( all_in SODINTYPE_FWSETSTATES not_in_state NOT_READY ) then
            	move_to ERROR
            endif
            move_to NOT_READY
...
objectset: FWCHILDREN_FWSETACTIONS union {SODINTYPE_FWSETACTIONS } is_of_class VOID
objectset: FWCHILDREN_FWSETSTATES union {SODINTYPE_FWSETSTATES } is_of_class VOID
